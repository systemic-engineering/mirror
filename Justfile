...
test:
    nix develop -c cargo test --package mirror --lib
...
coverage:
    nix develop -c cargo llvm-cov --package mirror --lib --fail-under-lines 76 --ignore-filename-regex 'story/|main\.rs|/nix/'

# HTML report
coverage-html:
    nix develop -c cargo llvm-cov --lib --html --open
