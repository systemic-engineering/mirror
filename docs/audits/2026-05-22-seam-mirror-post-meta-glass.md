...
1. **Prose inside `@nl` comments.** `# the grammar of mirror` → `# the glass of mirror`. Comments are prose; the meta-glass docstring claims the lift protects them; it doesn't.
...
**Remediation.** Pick one definition and use it from a single shared module. `bootstrap/src/spectral.rs::is_word_byte` is the older one; `apply_rewrites`'s narrower variant is the load-bearing one for path-component renames (the comment in `apply_rewrites` declaring `@` and `/` as boundaries is the intent). Lift it to `ast.rs` or a shared `word.rs`, re-import both call sites, write a property test asserting they agree on every byte in `0..=255`.
...
**The seam.** Every walker over `Combinator` or `AstNode` is a recursive descent with no explicit depth limit, no manual stack, and no compiler-enforced bound. `Combinator` itself has unbounded recursion through `Repeat::body`, `Capture::body`, `BraceBlock`, `ParenBlock`, `Until::stop`, `Lift::body`, and `Seq`/`Choice` children. `AstNode::children` is a `Vec` with no depth cap.
...
- **`@nl/code` lift semantics under hostile fenced-block tagging.** `nl.mirror` declares fenced blocks lift to `@code/<lang>` based on the tag immediately after ```` ``` ````. An attacker-controlled `.mirror` file could declare a fenced block with tag `mirror/grammar` (containing the meta-glass's own keywords) and trigger a recursive lift. The corresponding `Lift::body` walker is structural-self (F-1) today, so the immediate impact is bounded — but the moment the byte-consuming walker lands, the recursive-lift attack surface opens. Out of scope; flagged for the parser-aware audit at 4b.4.
