        ...
        _ => {
            ...
            if first_child.name == "variant" {
                ...
                let variants: Vec<&str> = children
                    ...
                    .collect();
                Some(format!("{} {} = {}", keyword, data.value, variants.join(" | ")))
                ...
                let params: Vec<&str> = children
                    ...
                    .map(|c| c.data().value.as_str())
                    .collect();
                Some(format!("{}{}({})", keyword, if data.value.is_empty() { String::new() } else { format!(" {}", data.value) }, params.join(", ")))
            } else {
...
mod tests {
    ...
    fn emit_parameterized() {
        let params = vec![
            ast::ast_leaf(Kind::Atom, "param", "precision", span()),
        ];
        let node = ast::ast_branch(Kind::Decl, "prism-def", "eigenvalues", span(), params);
        assert_eq!(emit_node(&node), Some("prism eigenvalues(precision)".to_string()));
    }
    ...
    fn emit_variants() {
        ...
        assert_eq!(emit_node(&node), Some("traversal color = red | blue | green".to_string()));
    }
    ...
    fn round_trip_resolved_fixtures() {
        let resolved_dir = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("fixtures/resolved");
        ...
        for entry in std::fs::read_dir(&resolved_dir).unwrap() {
            ...
            let ast = Parse.trace(source.clone()).into_result()
                .unwrap_or_else(|e| panic!("parse {}: {}", path.display(), e));
            let emitted = emit(&ast);
            assert_eq!(emitted, source, "resolved file {} does not round-trip", path.display());
        }
    }
    ...
    fn round_trip_boot() {
        ...
        for entry in std::fs::read_dir(&boot_dir).unwrap() {
            ...
            let expected: String = source
                ...
                .filter(|l| {
                    let t = l.trim();
                    t.starts_with("fold ") || t.starts_with("prism ") ||
                    t.starts_with("traversal ") || t.starts_with("lens ") ||
                    t.starts_with("iso ")
                })
                .map(|l| format!("{}\n", l))
                .collect();
            assert_eq!(emitted, expected, "round-trip failed for {}", path.display());
        }
    }
}
