...
fn parse_crystal_keyword(keyword: &str, rest: &str, lines: &mut Lines) -> Result<Prism<AstNode>, ParseError> {
    ...
    if let Some(eq_pos) = trimmed.find('=') {
        ...
        let variants: Vec<Prism<AstNode>> = variants_str
            ...
            .collect();
        lines.advance();
        return Ok(ast::ast_branch(Kind::Decl, &node_name, name, span, variants));
    }
