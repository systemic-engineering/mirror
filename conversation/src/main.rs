...
fn train_cmd() {
    ...
    let training_json = std::fs::read_to_string(&training_data_path)
        .unwrap_or_else(|e| { eprintln!("train: {}: {}", training_data_path.display(), e); process::exit(1); });
    ...
    for entry in &entries {
        ...
        let source = std::fs::read_to_string(&fixture_path)
            .unwrap_or_else(|e| { eprintln!("train: {}: {}", fixture_path.display(), e); process::exit(1); });

        // Parse and extract graph features
        let ast = Parse.trace(source).into_result()
            .unwrap_or_else(|e| { eprintln!("train: parse {}: {}", fixture_name, e); process::exit(1); });

        let graph: Vec<String> = ast.children().iter()
    ...
    std::fs::write(&weights_path, &bytes)
        .unwrap_or_else(|e| { eprintln!("train: write {}: {}", weights_path.display(), e); process::exit(1); });
    eprintln!("\n  wrote {} bytes to {}", bytes.len(), weights_path.display());
}
...
pub fn spectral_features(graph: &[String]) -> [f64; mirror::classifier::INPUT_DIM] {
    ...
    let traversal_count = graph.iter().filter(|s| s.starts_with("traversal-def:")).count() as f64;
    ...
    // 7: prefix entropy
    let prefixes: Vec<&str> = graph.iter()
    ...
    features[8] = if possible > 0 { edges as f64 / possible as f64 } else { 0.0 };

    // 9: param ratio — nodes containing parentheses in value
    let param_count = graph.iter().filter(|s| {
        s.split(':').nth(1).map_or(false, |v| v.contains('('))
    }).count() as f64;
    ...
    if fold_count > 0.0 { keyword_types += 1; }
    if prism_count > 0.0 { keyword_types += 1; }
    if traversal_count > 0.0 { keyword_types += 1; }
    if lens_count > 0.0 { keyword_types += 1; }
    if iso_count > 0.0 { keyword_types += 1; }
...
fn fmt_cmd(conv_path: &str, settle: bool, train: bool) {
    ...
    let ast = Parse.trace(source.clone()).into_result().unwrap_or_else(|e| {
    ...
    eprintln!("  {:?} ({:.0}%) {} → {} nodes",
        optic, confidence * 100.0, initial_len, resolved_len);
    ...
    if train {
        ...
        eprintln!("  train: loss={:.4} → {}", loss, weights_path.file_name().unwrap().to_str().unwrap());
    }
...
fn resolve_cmd() {
    ...
    let resolved_dir = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("fixtures/resolved");
    ...
    let mut entries: Vec<_> = std::fs::read_dir(&tension_dir)
        .unwrap_or_else(|e| { eprintln!("resolve: {}: {}", tension_dir.display(), e); process::exit(1); })
        .filter_map(|e| e.ok())
        .filter(|e| e.path().extension().and_then(|x| x.to_str()) == Some("conv"))
        .collect();
    ...
    for entry in &entries {
        ...
        let resolved_ast: PrismTree<AstNode> = match optic {
            ...
            _ => {
                ...
                ast_mod::ast_branch(
                    Kind::Decl, "root", "resolved",
                    Span::new(0, 0), children,
                )
            }
        };
        ...
        eprintln!("  {:25} {:?} ({:.0}%) {:2} → {:2} ({:+})",
            name, optic, confidence * 100.0,
            initial_len, resolved_len, -(reduced as i32));
