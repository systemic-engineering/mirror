...
mod tests {
    ...
    // -- Garden tests deleted 2026-04-07 --
    //
    // The eight `garden_*` tests previously here loaded `boot.backup/*.mirror`
    // and `garden/public/@training/training.mirror` to exercise the OLD
    // grammar pipeline. Both substrates are dead under the current
    // architecture (the OLD `Parse → Mirror::from_grammar` path is being
    // replaced by `mirror_runtime::parse_form`). Tests removed without
    // remorse; pattern is in git history if anyone needs it.
