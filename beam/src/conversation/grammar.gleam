...
fn find_grammar_start(
  ...
  case lines {
    ...
    [line, ..rest] -> {
      ...
      // Handle both `grammar @name {` and `abstract grammar @name {`
      ...
      case grammar_rest {
        Ok(after_at) -> {
          // "name {" or "name{" or "name extends @parent {"
          let without_brace =
            after_at
            |> string.replace(" {", "")
            |> string.replace("{", "")
            |> string.trim
          // Strip extends clause if present: "name extends @p1, @p2" → "name"
...
/// Starts at depth 1 (after the opening {). Stops when depth reaches 0.
