...
////
//// The boot sequence:
//// 1. Start @conversation actor (grammar compiler)
...
import conversation/conversation_actor
...
pub type BootResult {
  BootResult(
    compiler: Subject(conversation_actor.Message),
    domains: List(BootedDomain),
  )
}

/// Boot the runtime with a list of grammar sources.
/// Starts supervisor + @conversation actor, compiles each grammar, returns handles.
pub fn boot(grammars: List(String)) -> Result(BootResult, String) {
  ...
  // 2. Start @conversation actor
  case conversation_actor.start() {
    Error(_) -> Error("failed to start @conversation actor")
    Ok(started) -> {
      let compiler = started.data

      // 3. Compile each grammar
      case compile_all(compiler, grammars) {
        Ok(domains) ->
          Ok(BootResult(compiler: compiler, domains: domains))
        Error(e) -> {
          process.send(compiler, conversation_actor.Shutdown)
          Error(e)
        }
      }
    }
...
/// Compile a list of grammars through the @conversation actor.
fn compile_all(
  compiler: Subject(conversation_actor.Message),
  grammars: List(String),
) -> Result(List(BootedDomain), String) {
  compile_loop(compiler, grammars, [])
}
...
fn compile_loop(
  ...
  case remaining {
    [] -> Ok(list.reverse(acc))
    [source, ..rest] -> {
      case compile_one(compiler, source) {
        Ok(booted) -> compile_loop(compiler, rest, [booted, ..acc])
        Error(e) -> Error(e)
      }
    }
  }
...
fn compile_one(
  ...
  process.send(
    compiler,
    conversation_actor.CompileGrammar(source, reply),
  )
  case process.receive(reply, 10_000) {
    ...
    Ok(Ok(t)) -> {
      ...
      let beam_module = "conv_" <> compiled.domain
...
pub fn shutdown(result: BootResult) -> Nil {
  ...
  // Stop compiler
  process.send(result.compiler, conversation_actor.Shutdown)
}
